...
void MainWindow::on_label_customTripStadiumName_windowIconTextChanged(const QString &iconText)
{
	...
	// Defaulted to be false!!
/*	model->select();
	model->setHeaderData(1, Qt::Horizontal, QObject::tr("Team"));
	model->setHeaderData(2, Qt::Horizontal, QObject::tr("Item"));
	...
	ui->tableView_shoppingCart->hideColumn(2);
	souvenirModel->select();

	souvenirModel->submit();
	ui->tableView_shoppingCart->selectAll();
refresh();
	ui->tableView_customSouvenirView->setModel(model);
